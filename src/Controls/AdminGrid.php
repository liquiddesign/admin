...
class AdminGrid extends \Grid\Datagrid
{
	...
	{
		$grid = $this;
		$grid->getFilterForm()->addSubmit('submit', 'Filtrovat')->setHtmlAttribute('class', 'btn btn-sm btn-primary form-control-sm');

		$grid->getFilterForm()->onSuccess[] = function (Form $form) {
			$this->setPage(1);
		};

		$reset = $grid->getFilterForm()->addSubmit('reset', 'Zrušit')->setHtmlAttribute('class', 'btn btn-sm btn-secondary form-control-sm');
